...
function GroundDeposition({ pathOffset }: { pathOffset: number }) {
  ...
  useEffect(() => {
    ...
    const gradient = ctx.createRadialGradient(x, z, 0, x, z, 22);
    ...
    ctx.fillRect(0, 0, size, size);
    tex.needsUpdate = true;
    if (meshRef.current) (meshRef.current.material as THREE.MeshBasicMaterial).map = tex;
  }, [pathOffset, texture]);

  return (
    <mesh ref={meshRef} rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.445, 1]}>
      <planeGeometry args={[8, 8]} />
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {[
                ...
                "Ground splash al tocar el suelo",
                "Spray cloud volumetrico semitransparente",
                "Cultivos 3D en filas y camino de tierra",
                "Arboles perimetrales y vegetacion",
                "Camara orbital con drag, zoom y presets",
                "Presets de escenario (calmo, moderado, fuerte, extremo)",
                "Minimapa 2D con posicion del drone y cobertura",
                "Cobertura en tiempo real y estadisticas",
              ].map((item, i) => (
        ...
        <SectionReveal delay={0.2}>
          <div className="relative aspect-square max-w-lg mx-auto rounded-lg overflow-hidden border border-precisur-dark-600/30">
            ...
            <SceneHUD pathOffset={pathOffset} windSpeed={state.windSpeed} windDir={state.windDir}
              droneSpeed={state.droneSpeed} coverage={coverage} />
